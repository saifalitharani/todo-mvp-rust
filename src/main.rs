...
extern crate piaic_rust_assignment_3_todo;
extern crate diesel;

use self::diesel::prelude::*;
use self::piaic_rust_assignment_3_todo::*;
...
fn add_todo(t: Todo) {
    ...
    let todo_name: &str = &t.name[..];
    // DB insertion
    let todo = create_todo(&connection, todo_name);
    println!("\nSaved item {} with id {}", todo.name, todo.id);
...
fn remove_todo(idd: Uuid) {
    let connection = establish_connection();
    use piaic_rust_assignment_3_todo::schema::todos::dsl::*;

    let todoss = Arc::clone(&TODOS);
    let mut lock = todoss.write().unwrap();
    // find the index
    let mut idx = lock.len();
    for (i, todo) in lock.iter().enumerate() {
        if todo.id == idd {
            idx = i;
            let num_deleted = diesel::delete(todos.filter(name.eq(&todo.name[..])))
                .execute(&connection)
...
fn toggle_todo_done_db(todo : &mut Todo) {
    let connection = establish_connection();
    use piaic_rust_assignment_3_todo::schema::todos::dsl::*;
    let res = diesel::update(todos.filter(name.eq(&todo.name[..]))) 
        ...
        "svg" => {
            ...
            let body = {
                let xml = match file_name {
                    ...
                    "edit.svg" => include_str!("resource/edit.svg"),
                    _ => "",
                };
                Body::from(xml)
            };
    ...
    Box::new(
        req.into_body()
            ...
            .and_then(|whole_body| {
                ...
                //remove_todo_db(Todo::new(words[1]));
                redirect_home()
            }),
    )
}
...
fn edit_todo_handler(req: Request<Body>) -> ResponseFuture {
    Box::new(
        req.into_body()
            .concat2() // concatenate all the chunks in the body
            .from_err() // like try! for Result, but for Futures
            .and_then(|whole_body| {
                let str_body = String::from_utf8(whole_body.to_vec()).unwrap();
                let words: Vec<&str> = str_body.split('=').collect();
                edit_todo(Uuid::parse_str(words[1]).unwrap(),Todo::new(words[1]));
                redirect_home()
            }),
    )
}

fn deleteall_todo_handler(req: Request<Body>) -> ResponseFuture {
    Box::new(
        req.into_body()
            .concat2() // concatenate all the chunks in the body
            .from_err() // like try! for Result, but for Futures
...
fn router(req: Request<Body>, _client: &Client<HttpConnector>) -> ResponseFuture {
    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/delete_all") => deleteall_todo_handler(req),
